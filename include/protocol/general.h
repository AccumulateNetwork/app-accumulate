...
typedef struct FeeSchedule {
    ...
    // uint8_t fieldsSet[1];
    /// CreateIdentitySliding is the sliding fee schedule for creating an ADI. The first entry is
...
typedef struct IndexEntry {
    ...
    /// Anchor is the index of the chain into which the anchor was added. Omit when indexing the
    /// root anchor chain.
...
typedef struct Object {
    ...
    ChainMetadata
        *Chains;  //`json:"chains,omitempty" form:"chains" query:"chains" validate:"required"`
    /// Pending lists the account's pending transactions.
    TxIdSet
        Pending;  //`json:"pending,omitempty" form:"pending" query:"pending" validate:"required"`
                  // Bytes extraData[3];
...
typedef struct TxIdSet {
    ...
    Bytes32
        *Entries;  //`json:"entries,omitempty" form:"entries" query:"entries" validate:"required"`
                   // Bytes extraData[1];
...
ACME_API bool FactomDataEntryWrapper_equal(const FactomDataEntryWrapper *v,
                                           const FactomDataEntryWrapper *u);
...
ACME_API bool ValidatorPartitionInfo_equal(const ValidatorPartitionInfo *v,
                                           const ValidatorPartitionInfo *u);
...
ACME_API int unmarshalerReadAccountAuth(Unmarshaler *m, AccountAuth *v) {
...

    {
        ...
        v->Authorities =
            (AuthorityEntry *) unmarshalerAlloc(m, v->Authorities_length * sizeof(AuthorityEntry));
        for (size_t i = 0; i < v->Authorities_length; ++i) {
...
DataEntryType Accumulate_type(void) {
...
DataEntryType Factom_type(void) {
    return DataEntryTypeFactom;
}

/// Comparisons
bool FactomDataEntryWrapper_equal(const FactomDataEntryWrapper *v,
                                  const FactomDataEntryWrapper *u) {
    {
        const DataEntryType *a = &v->Type;
        const DataEntryType *b = &u->Type;
...
ACME_API int unmarshalerReadFeeSchedule(Unmarshaler *m, FeeSchedule *v) {
...

    {
        ...
        v->CreateIdentitySliding =
            (Fee *) unmarshalerAlloc(m, v->CreateIdentitySliding_length * sizeof(Fee));
        for (size_t i = 0; i < v->CreateIdentitySliding_length; ++i) {
...
ACME_API int unmarshalerReadNetworkDefinition(Unmarshaler *m, NetworkDefinition *v) {
    ...
    }
    {
        ...
        // now unmarshal for real...
        v->Partitions =
            (PartitionInfo *) unmarshalerAlloc(m, v->Partitions_length * sizeof(PartitionInfo));
        for (size_t i = 0; i < v->Partitions_length; ++i) {
    ...
    }
    {
        ...
        // now unmarshal for real...
        v->Validators =
            (ValidatorInfo *) unmarshalerAlloc(m, v->Validators_length * sizeof(ValidatorInfo));
        for (size_t i = 0; i < v->Validators_length; ++i) {
...
ACME_API int unmarshalerReadRoutingTable(Unmarshaler *m, RoutingTable *v) {
...

    {
        ...
        // now unmarshal for real...
        v->Overrides =
            (RouteOverride *) unmarshalerAlloc(m, v->Overrides_length * sizeof(RouteOverride));
        for (size_t i = 0; i < v->Overrides_length; ++i) {
...
ACME_API int unmarshalerReadValidatorInfo(Unmarshaler *m, ValidatorInfo *v) {
    ...
    }
    {
        ...
        // now unmarshal for real...
        v->Partitions = (ValidatorPartitionInfo *) unmarshalerAlloc(
            m,
            v->Partitions_length * sizeof(ValidatorPartitionInfo));
        for (size_t i = 0; i < v->Partitions_length; ++i) {
...
/// Comparisons
bool ValidatorPartitionInfo_equal(const ValidatorPartitionInfo *v,
                                  const ValidatorPartitionInfo *u) {
    {
        const String *a = &v->ID;
        const String *b = &u->ID;
