...
bool Bytes_equal(const Bytes *b1, const Bytes *b2) {
    ...
    return memcmp(b1->buffer.ptr + b1->buffer.offset,
                  b2->buffer.ptr + b2->buffer.offset,
                  b1->buffer.size) == 0;
}
...
int Bytes_copy(Bytes *dst, const Bytes *src) {
    ...
    memmove((uint8_t *) (dst->buffer.ptr + dst->buffer.offset),
            src->buffer.ptr + src->buffer.offset,
            dst->buffer.size);
