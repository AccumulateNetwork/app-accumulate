...
Error MerkleDAGRoot(MerkleState *m, uint8_t mdroot[static 32]) {
    ...
    for (int i = 0; i < m->pendingLen; i++) {  //_, v := range m.Pending {
        if (first &&
            m->pending[i].size != 0) {  // Pick up the first hash in m.MerkleState no matter what.
            first = false;
            memmove(mdroot, m->pending[i].ptr + m->pending[i].offset, m->pending[i].size);
            // = Hash(v).Copy() // If a nil is assigned over a nil, no harm no foul.  Fewer cases to
            // test this way.
        } else if (!first &&
                   m->pending[i].size != 0) {  // If MDRoot isn't nil and v isn't nil, combine them.
...
static int hasherAppend(MerkleState *m, const Bytes *v) {
    uint8_t hash[32];
    sha256(v->buffer.ptr + v->buffer.offset, v->buffer.size - v->buffer.offset, hash, sizeof(hash));
...
int hasherAddInt(MerkleState *m, int64_t v) {
    ...
    return hasherAppend(m,
                        &(const Bytes){.buffer.ptr = b, .buffer.offset = 0, .buffer.size = size});
}
